...
def main():
    ...
    p.add_argument("--init-from-adapter", default=None,
                   help="Path to existing PEFT adapter to initialize from (M31 2-stage curriculum).")
    ...
    if args.init_from_adapter:
        ...
        print(f"[xv-onpolicy-b] init from adapter: {args.init_from_adapter}", flush=True)
        student = PeftModel.from_pretrained(student, args.init_from_adapter, is_trainable=True)
    else:
